...
bool Individual::is_valid_order()
{
    ...
    for (int i = 0; i < NUM_OF_CUSTOMERS; i++)
    {
        if (order[i] < 1 || order[i] > NUM_OF_CUSTOMERS)
            return false;
        if (!have[order[i]])
        {
            have[order[i]] = 1;
...
void Individual::redistribute_customer()
{
    ...
    for (int x : nearest[customer])
    {
        ...
        // Better ?
        if (cap1 + get_customer_demand(x) <= MAX_CAPACITY && abs(cap1 + get_customer_demand(x) - (cap2 - get_customer_demand(x))) < abs(cap1 - cap2))
        {
...
void Individual::optimize_station(int *full_path, int l, int r)
{
    ...
    for (int i = r; i - 2 > l; i--)
    {
        ...
        deltaL1 = get_distance(full_path[i], full_path[i - 1]) + get_distance(full_path[i - 1], full_path[i - 2]) - get_distance(full_path[i], full_path[i - 2]);
...
void Individual::mutation()
{
    ...
    if (mutate_prob_2 < Pr_mutate)
    {
        ...
        for (int x : nearest[customer])
        {
            if (tour_index[x] != tour_index[customer] && cost + get_customer_demand(x) <= MAX_CAPACITY && tours[tour_index[x]].right - tours[tour_index[x]].left > 1)
            {
